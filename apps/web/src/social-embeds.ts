...
export function socialEmbedFromUrl(raw: string): EmbedReference | undefined {
  let url: URL;
  try {
  ...
  return youtubeEmbed(url) ?? xEmbed(url);
}
...
export function installSocialEmbedHydration(root: HTMLElement): () => void {
...
function youtubeEmbed(url: URL): EmbedReference | undefined {
...
function xEmbed(url: URL): EmbedReference | undefined {
  const host = url.hostname.toLowerCase();
